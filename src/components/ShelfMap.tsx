...
export function ShelfMap({ onSelectProduct, highlightProductId, refreshKey = 0 }: ShelfMapProps) {
  ...
  const [productosEstante, setProductosEstante] = useState<Producto[]>([]);
  ...
  useEffect(() => {
    ...
    void getCuadrantes(estanteActivo).then(async (cuads) => {
      ...
      setProductosCuadrante([]);

      if (cuads.length === 0) {
        const prods = await getProductosPorEstante(estanteActivo);
        setProductosEstante(prods);
        setOcupacion({});
        return;
      }
      ...
      for (const c of cuads) {
        const cuadranteProducts = await getProductosPorCuadrante(c.id);
        counts[c.id] = cuadranteProducts.length;
      }
      setOcupacion(counts);
    });
  }, [estanteActivo]);
  ...
  const usaCuadrantes = cuadrantes.length > 0;
        ...
        <div>
          ...
          {!usaCuadrantes && (
            <p className="mt-1 text-xs text-[var(--text-muted)]">
              {productosEstante.length} productos con ubicación descriptiva del inventario real
            </p>
          )}
        </div>
